    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (state)
                ...
                S_TILE_LOAD: begin
                    // Load 2 words of activations and 16 words of weights (for ROWS=8)
                    // Request weight word 's' at sub_cnt = s
                    // Capture weight word 's' at sub_cnt = s + 2
                    
                    // 1. Requests
                    if (sub_cnt < 2*ROWS) begin
                        // Activations: only first 2 words
                        if (sub_cnt == 0) in_raddr <= col_tile;
                        if (sub_cnt == 1) in_raddr <= col_tile + 1;
                        
                        // Weights: 2 words for each of the 8 rows = 16 words
                        wt_raddr <= cur_wt_base[14:0] + (row_tile + (sub_cnt >> 1)) * (cur_cols >> 2) + col_tile + (sub_cnt & 1);
                    end

                    // 2. Captures
                    if (sub_cnt == 2) systolic_a_data[31:0] <= in_rdata;
                    if (sub_cnt == 3) systolic_a_data[63:32] <= in_rdata;
                    
                    if (sub_cnt >= 2 && sub_cnt <= 2*ROWS + 1) begin
                    ...
                    // 3. State transition
                    if (sub_cnt == 2*ROWS + 1) begin
                        systolic_w_load <= 1;
                        systolic_a_valid <= 1;
                        sub_cnt <= 0;
                        if (col_tile + 2 < (cur_cols >> 2)) begin
                            col_tile <= col_tile + 2;
                            state <= S_TILE_LOAD;
                        end else begin
                ...
                S_COLLECT_WAIT: if (systolic_valid_out) begin state <= S_COLLECT_WRITE; sub_cnt <= 0; end
    ...
    always @(*) begin
        ...
        if (cop_re) begin
            case (cop_addr[11:0])
                ...
                12'h004: cop_rdata = {31'd0, (state != S_IDLE)};
