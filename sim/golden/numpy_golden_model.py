...
def int8_mac_relu(vec_a, vec_b, bias):
    ...
    # 3. Fused ReLU (Hardware unconditionally clamps negative results to 0)
    result = max(0, acc)
    return result
...
def full_inference(image_784_int8, W1_int8, b1_int32, W2_int8, b2_int32):
    ...
    # LAYER 2: 128 -> 10 
    # FIXED: Replaced raw dot product with systolic_layer.
    # The hardware vec_mac_engine unconditionally applies ReLU, so 
    # Layer 2 must also undergo the exact same ReLU clamping to match.
    layer2_out = systolic_layer(layer1_out, W2_int8, b2_int32)
